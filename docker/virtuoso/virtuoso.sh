...
isql-vt localhost:1111 dba dba /1_create_users.sql
isql-vt localhost:1111 dba dba /2_setup_full_text_indexing.sql
kill $(ps aux | grep '[v]irtuoso-t' | awk '{print $2}')
virtuoso-t +wait +foreground +configfile /virtuoso.ini

#virtuoso-t +wait && isql-vt -U dba -P dba < /dump_nquads_procedure.sql && isql-vt -U dba -P dba < /sql-query.sql
...
#    virtuoso-t +wait && isql-vt -U dba -P "$pwd" < /load_data.sql
#    kill $(ps aux | grep '[v]irtuoso-t' | awk '{print $2}')
