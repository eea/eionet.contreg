...
public class UserFolderDAOTest {
    ...
    public void testGetParameters() {
        ...
        assertEquals(" . ?s a ?type . FILTER (?type  IN (?crFile, ?crUserFolder)) . ?parentFolder ?hasPredicate ?s . "
                + "FILTER (?hasPredicate IN (?hasFile, ?hasFolder))", params);
    ...
    public void testUnorderedQuery() {
        ...
        assertEquals("DEFINE input:inference'CRInferenceRule' PREFIX cr: <http://cr.eionet.europa.eu/ontologies/contreg.rdf#> "
                ...
                + "FILTER (?hasPredicate IN (?hasFile, ?hasFolder))}", query);
