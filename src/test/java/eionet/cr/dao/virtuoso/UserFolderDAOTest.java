...
public class UserFolderDAOTest {
    ...
    public void testGetParameters() {
        ...
        assertEquals(
                " . ?s a ?type . FILTER (?type  IN (?crFile, ?crUserFolder)) . ?parentFolder ?hasPredicate ?s . FILTER (?hasPredicate IN (?hasFile, ?hasFolder))",
                params);
    ...
    public void testCountQuery() {
        ...
        assertEquals(
                "DEFINE input:inference'CRInferenceRule' select count(distinct ?s) where { ?s ?p ?o  . ?s a ?type . FILTER (?type  IN (?crFile, ?crUserFolder)) . ?parentFolder ?hasPredicate ?s . FILTER (?hasPredicate IN (?hasFile, ?hasFolder))}",
                query);
    }
    ...
    public void testUnorderedQuery() {
        ...
        assertEquals(
                "DEFINE input:inference'CRInferenceRule' PREFIX cr: <http://cr.eionet.europa.eu/ontologies/contreg.rdf#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> select distinct ?s where { ?s ?p ?o  . ?s a ?type . FILTER (?type  IN (?crFile, ?crUserFolder)) . ?parentFolder ?hasPredicate ?s . FILTER (?hasPredicate IN (?hasFile, ?hasFolder))}",
                query);
