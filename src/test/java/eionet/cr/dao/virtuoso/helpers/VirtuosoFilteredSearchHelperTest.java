...
//                + "?o1 = \"http://rod.eionet.europa.eu/schema.rdf#Obligation\")}}} limit 15 offset 0");
    ...
    public void testOrderedQuery() {
        PagingRequest pagingRequest =  PagingRequest.create(1);
        SortingRequest sortingRequest = new SortingRequest("http://www.w3.org/2000/01/rdf-schema#label", 
...
//                + "bif:strrchr (bif:replace (?s, '/', '#'), '#')+1))) , bif:lcase(?oorderby)))");
    ...
    public void testUnOrderedQuery() {
        PagingRequest pagingRequest =  PagingRequest.create(1);
        SortingRequest sortingRequest = new SortingRequest(null, 
                SortOrder.parse(SortOrder.ASCENDING.toString()));
        
        Map<String, String> filters = new HashMap<String, String>();
        //http://rod.eionet.europa.eu/schema.rdf#obligation=http://rod.eionet.europa.eu/obligations/15, http://www.w3.org/2000/01/rdf-schema#label=CLRTAP}
...
//                +"{{?s ?p3 ?o3 . ?s <http://www.w3.org/2000/01/rdf-schema#label> ?o3} . { ?s ?p3 ?o3 . FILTER bif:contains(?o3, \"'CLRTAP'\")}}");
    ...
    public void testDateSorting() {
        PagingRequest pagingRequest =  PagingRequest.create(1);
        SortingRequest sortingRequest = new SortingRequest("http://cr.eionet.europa.eu/ontologies/contreg.rdf#contentLastModified", 
                SortOrder.parse(SortOrder.DESCENDING.toString()));
        ...
        String query = helper.getOrderedQuery(inParams);
        assertEquals("DEFINE input:inference'CRInferenceRule' select distinct ?s max(?time) AS ?oorderby where {graph ?g { ?s ?p ?o  . {{?s ?p1 ?o1 . ?s ?predicateValue1 ?o1} . { ?s ?p1 ?o1 . FILTER bif:contains(?o1, ?objectValue1)}} . OPTIONAL {?g ?sortPredicateValue ?time}}} ORDER BY desc(?oorderby)", query);
    }
