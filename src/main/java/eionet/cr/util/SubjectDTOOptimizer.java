                ...
                for (ObjectDTO object : foundResources) {
                    ...
                    for (int a = 0; a < languages.size(); a++) {
                        for (ObjectDTO labelObject : objects) {
                            if (labelObject.getSourceObjectHash() == object.getHash() && labelObject.isLiteral()
                                    && labelObject.getLanguage().equals(languages.get(a)) && suitableLabelFound == false) {
                ...
                for (int a = 0; a < languages.size(); a++) {
                    for (ObjectDTO object : objects) {
                        if (object.isLiteral() && object.getSourceObjectHash() == 0
                                && object.getLanguage().equals(languages.get(a))) {

                            // Now testing whether an object with exactly the same literal value is already present (with higher
                            // ranked language).
