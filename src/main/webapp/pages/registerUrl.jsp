...
// ]]>
    ...
    <stripes:layout-component name="contents">
        ...
        <c:if test="${ actionBean.showFactsheetLink }">

            <c:url value="factsheet.action" var="sourceUrl">
                <c:param name="uri" value="${ actionBean.url }" />
                <c:param name="view" value="" />
            </c:url>
            <div style="margin-top:20px" class="note-msg">
                <strong>Know url</strong>
                <p>${ actionBean.registrationMessage }
                    <c:choose>
                        <c:when test="${not empty actionBean.url}">
                            <stripes:link href="${ sourceUrl }"><c:out value="${actionBean.url}"/></stripes:link>
                        </c:when>
                        <c:otherwise>
                            ${actionBean.uri}
                        </c:otherwise>
